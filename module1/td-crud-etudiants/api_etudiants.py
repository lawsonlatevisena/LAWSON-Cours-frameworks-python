...
class Etudiant(db.Model):
    ...
    notes     = db.relationship('Note', backref='etudiant',
    ...
    def to_dict(self, with_notes=False):
        d = {
            'id': self.id, 'matricule': self.matricule,
            'nom': self.nom, 'prenom': self.prenom,
            'email': self.email, 'filiere': self.filiere,
            'annee': self.annee, 'moyenne': self.moyenne,
            'date_inscription': self.date_insc.isoformat()
        }
        if with_notes:
...
class Note(db.Model):
    ...
    def to_dict(self):
        return {
            'id': self.id, 'matiere': self.matiere,
            'valeur': self.valeur, 'coefficient': self.coefficient,
            'date_examen': self.date_examen.isoformat()
        }
...
def lister_etudiants():
    """GET /api/etudiants?filiere=Maths&annee=2&q=Koffi&page=1&per_page=10"""
    ...
    if recherche:
        t = f'%{recherche}%'
        q = q.filter(db.or_(Etudiant.nom.ilike(t), Etudiant.prenom.ilike(t),
                             Etudiant.matricule.ilike(t)))
...
def get_etudiant(eid):
    return jsonify(db.get_or_404(Etudiant, eid).to_dict(with_notes=True)), 200
...
def modifier_etudiant(eid):
    etudiant = db.get_or_404(Etudiant, eid)
    data = request.get_json()
    try:
...
def get_notes(eid):
    e = db.get_or_404(Etudiant, eid)
    return jsonify({'etudiant': f'{e.prenom} {e.nom}',
                    'notes': [n.to_dict() for n in e.notes.all()],
                    'moyenne': e.moyenne}), 200
...
def ajouter_note(eid):
    e = db.get_or_404(Etudiant, eid)
    try:
        valide = note_schema.load(request.get_json())
    except ValidationError as err:
        return erreur('Donnees invalides', 422, err.messages)
    note = Note(**valide, etudiant_id=eid)
    db.session.add(note)
    db.session.commit()
    return jsonify({'note': note.to_dict(), 'nouvelle_moyenne': e.moyenne}), 201
